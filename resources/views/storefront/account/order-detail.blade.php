...
@section('account-content')
    ...
    <section class="account-content">
        ...
        <section class="account-card">
            <div class="detail-top"><div><h2>سفارش #{{ $order['order_number'] }}</h2><p>ثبت شده در {{ \App\Support\JalaliDate::format($order['created_at'], 'j F Y H:i') }}</p></div><div class="detail-top__meta"><span class="status-badge status-badge--info">{{ $order['status']['label'] }}</span><strong>{{ \App\Support\PersianNumber::money($order['totals']['grand_total']) }}</strong></div></div>
            @if ($order['timeline'] !== [])
                ...
                <div class="order-products">
                    @foreach ($order['items'] as $item)
                        <div class="detail-product">
                            ...
                            <span><strong>@if ($item['url'])<a href="{{ $item['url'] }}">{{ $item['name'] }}</a>@else{{ $item['name'] }}@endif</strong><small>{{ $item['sku'] ? 'SKU: '.$item['sku'].' · ' : '' }}تعداد {{ \App\Support\PersianNumber::digits($item['quantity']) }}@foreach ($item['variation_attributes'] as $attribute => $value) · {{ $attribute }}: {{ $value }}@endforeach</small></span>
                            <b>{{ \App\Support\PersianNumber::money($item['line_total']) }}</b>
                        </div>
                    @endforeach
                </div>
            </section>
            <div class="detail-stack">
                <section class="account-card"><div class="account-card__heading"><h2>مبلغ سفارش</h2></div><div class="detail-totals"><div class="detail-total-row"><span>جمع کالاها</span><strong>{{ \App\Support\PersianNumber::money($order['totals']['items_subtotal']) }}</strong></div><div class="detail-total-row"><span>تخفیف</span><strong>{{ \App\Support\PersianNumber::money($order['totals']['discount_total']) }}</strong></div><div class="detail-total-row"><span>مالیات</span><strong>{{ \App\Support\PersianNumber::money($order['totals']['tax_total']) }}</strong></div><div class="detail-total-row"><span>هزینه ارسال</span><strong>{{ \App\Support\PersianNumber::money($order['totals']['shipping_total']) }}</strong></div><div class="detail-total-row detail-total-row--grand"><span>مبلغ نهایی</span><strong>{{ \App\Support\PersianNumber::money($order['totals']['grand_total']) }}</strong></div></div></section>
        ...
        @if ($order['coupon'])<section class="account-card"><div class="account-card__heading"><h2>تخفیف سفارش</h2></div><p>کد تخفیف: <strong>{{ $order['coupon']['code'] }}</strong> · {{ \App\Support\PersianNumber::money($order['coupon']['discount_amount']) }}</p></section>@endif
        <section class="account-card"><div class="account-card__heading"><h2>اطلاعات ارسال</h2></div><dl class="info-list">@if ($order['shipping'])<div><dt>روش ارسال</dt><dd>{{ $order['shipping']['service'] ?? '—' }}</dd></div><div><dt>هزینه ارسال</dt><dd>{{ \App\Support\PersianNumber::money($order['shipping']['amount']) }}</dd></div>@endif @if ($order['shipping_address'])<div><dt>گیرنده</dt><dd>{{ $order['shipping_address']['first_name'] }} {{ $order['shipping_address']['last_name'] }} · {{ \App\Support\PersianNumber::digits($order['shipping_address']['mobile']) }}</dd></div><div><dt>نشانی</dt><dd>{{ $order['shipping_address']['province_name'] }}، {{ $order['shipping_address']['city_name'] }}، {{ $order['shipping_address']['address_line'] }}</dd></div>@if ($order['shipping_address']['postal_code'])<div><dt>کد پستی</dt><dd>{{ \App\Support\PersianNumber::digits($order['shipping_address']['postal_code']) }}</dd></div>@endif @endif</dl></section>
