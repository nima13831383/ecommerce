...
@section('account-content')
    ...
    <section class="account-content">
        ...
        <section class="account-card">
            <div class="detail-top"><div><h2>سفارش #{{ $order['order_number'] }}</h2><p>ثبت شده در {{ $order['created_at']?->format('Y/m/d H:i') }}</p></div><div class="detail-top__meta"><span class="status-badge status-badge--info">{{ $order['status']['label'] }}</span><strong>{{ number_format($order['totals']['grand_total']) }} ریال</strong></div></div>
            @if ($order['timeline'] !== [])
