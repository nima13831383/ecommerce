...
final class SettingRegistry
{
    ...
    public static function definitions(): array
    {
        return [
            ...
            'cache.store' => new SettingDefinition(
                key: 'cache.store',
                group: 'cache',
                type: 'string',
                default: 'database',
                label: 'محل ذخیره کش فروشگاه',
                rules: ['required', Rule::in(['database', 'redis'])],
                options: ['database' => 'دیتابیس', 'redis' => 'ردیس'],
                description: 'فقط کش کوئری‌های عمومی فروشگاه را انتخاب می‌کند؛ اطلاعات اتصال Redis در ENV قرار دارد.',
            ),
            'cache.products.ttl_seconds' => new SettingDefinition(
                key: 'cache.products.ttl_seconds',
                group: 'cache',
                type: 'integer',
                default: 3600,
                label: 'اعتبار کش محصولات (ثانیه)',
                rules: ['required', 'integer', 'min:60', 'max:86400'],
            ),
            'cache.blog.ttl_seconds' => new SettingDefinition(
                key: 'cache.blog.ttl_seconds',
                group: 'cache',
                type: 'integer',
                default: 3600,
                label: 'اعتبار کش مقالات (ثانیه)',
                rules: ['required', 'integer', 'min:60', 'max:86400'],
            ),
            'cache.lock_seconds' => new SettingDefinition(
                key: 'cache.lock_seconds',
                group: 'cache',
                type: 'integer',
                default: 60,
                label: 'مدت قفل بازسازی کش (ثانیه)',
                rules: ['required', 'integer', 'min:60', 'max:600'],
            ),
            'cache.stale_seconds' => new SettingDefinition(
                key: 'cache.stale_seconds',
                group: 'cache',
                type: 'integer',
                default: 86400,
                label: 'مدت سرویس داده قدیمی کش (ثانیه)',
                rules: ['required', 'integer', 'min:60', 'max:604800'],
            ),
            'cache.refresh_ahead.enabled' => new SettingDefinition(
                key: 'cache.refresh_ahead.enabled', group: 'cache', type: 'boolean', default: true,
                label: 'بازسازی پیشگیرانه صفحات پرترافیک', rules: ['required', 'boolean'],
                description: 'فقط آرشیو پیش‌فرض صفحه اول محصولات و مجله را نزدیک پایان اعتبار در صف کم‌اولویت بازسازی می‌کند.',
            ),
            'cache.refresh_ahead_percent' => new SettingDefinition(
                key: 'cache.refresh_ahead_percent', group: 'cache', type: 'integer', default: 15,
                label: 'پنجره بازسازی پیشگیرانه (درصد)', rules: ['required', 'integer', 'min:5', 'max:30'],
                description: 'درصد پایانی عمر داده تازه که درخواست، بازسازی پس‌زمینه را پیشنهاد می‌کند.',
            ),
