...
test('static storefront pages render the raw design content and real navigation', function (): void {
    $this->get(route('storefront.about'))->assertOk()->assertSee('درباره لوکسیر')->assertSee(route('storefront.blog.index'));
    $this->get(route('storefront.contact'))->assertOk()->assertSee('تماس با ما')->assertSee('فرم پیام در حال حاضر');
    $this->get(route('storefront.faq'))->assertOk()->assertSee('پرسش‌های متداول')->assertSee('چطور سفارش ثبت کنم؟');
});
